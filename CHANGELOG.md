...
* **`js/locales/*.js`**：教學文字綁在陣列索引（`tutorial.stepN`），四語系 step1~step6 同步輪轉，使文字與高光對齊。
* **驗證**：本機實機啟動教學——逐一核對 10 步 `tutorial.stepN` 文字與 highlight／waitFor 對齊；枷鎖現於第 7/10 步（攻擊前）；step0=敵人、step1=回合（原為枷鎖），實機推進確認。

### 平衡文案：絕對秩序說明改「八顆全奇或全偶」[2026/07/10]
* **需求**：原「7顆以上」不直覺——基礎門檻其實是八顆，7／6 顆是靠遺物【寬容】／【天秤之極】才降低。
* **`js/data.js` / `js/locales/*.js`**：`rule_d1` 說明改為「八顆數字全為奇數或全為偶數」（簡：八颗数字全为奇数或全为偶数；英：All 8 dice are odd, or all even；日：8個のダイスがすべて奇数、またはすべて偶数）。
* **`js/ui.js`**：牌型表 `rule_d1` 範例骰由「7 奇＋?」改為「8 顆全奇」以符合基礎規則。
* **驗證**：`steam:i18n:verify` 四語 716 keys 對齊；未動 `engine.js` 判定（門檻邏輯不變）。
